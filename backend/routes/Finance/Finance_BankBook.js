...
//read data
router.route("/Finance_BankBook").get(async (req, res) => {
  try {
    let db = DB.getDB();
    let result = await db.collection("Finance_BankBook").find({}).toArray();
...
router.route("/Finance_BankBook/:id").get(async (req, res) => {
  try {
    let db = DB.getDB();
    let result = await db
...
//delete data
router.route("/Finance_BankBook/:id").delete(async (req, res) => {
  try {
    let db = DB.getDB();
    let data = await db
      .collection("Finance_BankBook")
...
//insert data
router.route("/Finance_BankBook").post(async (req, res) => {
  try {
    let db = DB.getDB();
