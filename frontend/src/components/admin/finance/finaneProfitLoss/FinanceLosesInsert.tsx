...
const FinanceLosesInsert = () => {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className={`sm:max-w-[425px] border-2 ${errors.amount_expense || errors.type || errors.category || errors.description ? "border-destructive/50" : ""}`}>
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit(newTransaction)}>
          <div className="grid gap-4 py-4">
            ...
            {/* Amount */}
            <div className="grid grid-cols-4 items-center gap-x-4">
              <Label htmlFor="amount_expense" className="text-right">Amount</Label>
              <Input
                id="amount_expense"
                placeholder="0.00"
                className="col-span-3"
                {...register("amount_expense", {
                  required: "Amount is required",
                  min: { value: 0, message: "Amount should be greater than 0" },
                  pattern: { value: /^\d+(\.\d{1,2})?$/, message: "Invalid amount format" },
                })}
              />
              {errors.amount_expense && <span className="text-destructive text-sm col-span-3">{errors.amount_expense.message}</span>}
            </div>
          </div>
          <DialogFooter>
            <Button
              className={`cursor-pointer w-full ${errors.amount_expense || errors.type || errors.category || errors.description ? "bg-destructive/50 hover:bg-destructive/70 cursor-not-allowed animate-pulse" : ""}`}
              type="submit"
