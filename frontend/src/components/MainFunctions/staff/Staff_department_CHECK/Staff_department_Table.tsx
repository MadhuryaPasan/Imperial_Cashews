...
const table = ({ selectedMonth }: any) => {
  ...
  useEffect(() => {
    async function getAll() {
      let result = await Staff_department_getAllData();
      setRow(result);
    }
    getAll();
  }, []);
              ...
              {row
                ...
                .map((rowData: any) => (
                  <TableRow key={rowData._id} className="hover:bg-primary/10">
                    ...
                    <TableCell>{rowData.manager}</TableCell>
                    <TableCell>{rowData.staffCount}</TableCell>
...
import { Staff_department_getAllData, Staff_department_deleteDoc } from "@/utils/staff/Staff_department_API";
import Staff_department_Update from "../Staff_department_CHECK/Staff_department_Update"
  ...
  return (
    <>
      <Dialog>
        ...
        <DialogContent>
          <div>
            {/* {currentData ? <Staff_department_Update {...currentData} /> : <p>Loading...</p>} */}
            <Staff_department_Update currentData={updateId}/>
          </div>
        </DialogContent>
      </Dialog>
    </>
...
const deleteBtn = (deleteId: any) => {
  ...
  const deleteOne = async (id: string) => {
    await Staff_department_deleteDoc(id);
...
import Staff_department_insert from '@/components/MainFunctions/staff/Staff_department_CHECK/Staff_department_insert'
const insertBtn = () => {
  return (
    <>
      <Dialog>
